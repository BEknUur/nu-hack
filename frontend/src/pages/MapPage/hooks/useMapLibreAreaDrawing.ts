...
export function useMapLibreAreaDrawing({
  ...
  useEffect(() => {
    ...
    const onMouseMove = (event: maplibregl.MapMouseEvent) => {
      if (!isMouseDown) {
        if (drawMode === 'polygon' && polygonPoints.length >= 2) {
          onPreview(polygonFromVertices([...polygonPoints, [event.lngLat.lng, event.lngLat.lat]]));
        }
        return;
      }

      if (drawMode === 'rectangle' && startPoint) {
        onPreview(rectangleToPolygon(startPoint, [event.lngLat.lng, event.lngLat.lat]));
      }
      ...
      if (drawMode === 'circle' && startPoint) {
        ...
        onPreview(circleToPolygon(startPoint, Math.max(4, radiusMeters)));
      }
      ...
      if (drawMode === 'freehand') {
        ...
        if (!last || maplibregl.LngLat.convert(last).distanceTo(maplibregl.LngLat.convert(point)) > 4) {
          ...
          onPreview(freehandToPolygon(freehandPoints));
        }
      }
    };
    ...
    const onClick = (event: maplibregl.MapMouseEvent) => {
      ...
      onPreview(polygonFromVertices(polygonPoints));
    };
  ...
  }, [drawMode, enabled, onBegin, onFinish, onPreview, rawMapRef]);
}
